...
| Student onboarding | `BLOCKED` | No local student list/create API or web create/select workflow; extension requires a profile ID | Development seed covers only the deterministic synthetic profile | A genuinely fresh participant has no supported way to establish a real local identity | Define and implement a source-tagged institution/student onboarding contract without guessing school data |
| MyProgress import | `BLOCKED` | staging/review/apply APIs and parsers exist, but all routes require an existing student profile | Synthetic API/extension/browser tests; packaged synthetic import reached apply | No supported fresh-profile bootstrap and no authorized participant source run | Implement onboarding, then execute controlled real source after participant login |
...
2. A genuinely fresh local database has no supported student-profile creation/selection workflow. The deterministic development seed cannot stand in for a participant identity or reviewed institution mapping.
3. MyProgress, Program Catalog, Sections, extension capture, and downstream academic/optimizer real-source acceptance require the onboarding gap to be closed plus a participant-owned authenticated session and explicit authorization. No credentials may be requested, stored, logged, or pasted.
